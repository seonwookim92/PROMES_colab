...
def get_reward(env_prev, cluster, action, info, time, debug=False):
    ...
    reward, pwd, rbd1, rbd2 = reward_helper(env_prev, cluster, action, info, time, debug=False)
    d_reward, d_pwd, d_rbd1, d_rbd2 = reward_helper(env_prev, default_cluster, default_action, default_info, default_time, debug=False)

    f_pwd = pwd - d_pwd
    f_rbd1 = rbd1 - d_rbd1
    f_rbd2 = rbd2 - d_rbd2

    reward = w1 * f_pwd + w2 * f_rbd1 + w3 * f_rbd2
    reward = round(reward, 2)

    return reward

def reward_helper(env_prev, cluster, action, info, time, debug=False):
